...
class ActivityDto(BaseModel):
    ...
    activity_type: str = Field(default="activity", alias="activityType", pattern=r"^(place|activity|event)$")
    map_url: str | None = Field(default=None, alias="mapUrl", max_length=500)
    web_url: str | None = Field(default=None, alias="webUrl", max_length=500)
...
async def create_activity(conn: AsyncConnection, activity: Activity) -> Activity:
	values = activity_to_insert_values(activity)

	stmt = insert(activities).values(**values).returning(
		activities.c.id,
		activities.c.version,
		activities.c.created_at,
		activities.c.updated_at,
	)
